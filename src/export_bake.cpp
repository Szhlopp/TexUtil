...
Json parseExportBake(const Json& input, const std::filesystem::path& base) {
    ...
    out["size"]=integer(out.value("size",Json(1024)),32,4096,"size");out["padding"]=integer(out.value("padding",Json(8)),0,64,"padding");out["normal_convention"]=out.value("normal_convention",Json("directx"));require(out["normal_convention"]=="directx"||out["normal_convention"]=="opengl","normal_convention must be directx or opengl");
