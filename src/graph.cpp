...
Graph::Graph(Json doc, std::filesystem::path base, Options options) : base_(std::move(base)), options_(std::move(options)) {
    ...
    for(const auto& bake:outputs_)if(bake.bake){auto path=std::filesystem::path(bake.name);auto directory=std::filesystem::weakly_canonical(options_.out/path.parent_path()/(path.stem().string()+".assets"));
        ...
        auto protect=[&](const std::filesystem::path& input){auto canonical=std::filesystem::weakly_canonical(input);auto rel=canonical.lexically_relative(directory);require(rel.empty()||*rel.begin()=="..","bake package overlaps an input asset");for(const auto& output:outputs_)require(canonical!=std::filesystem::weakly_canonical(options_.out/output.name),"output would overwrite bake recipe asset");};
        protect(bake.bake->at("model").get<std::string>());for(const auto& file:expanded.files)protect(file);for(const auto& [id,node]:nodes_)if(node["op"]=="image")protect(base_/node["path"].get<std::string>());
        std::vector<Json> bindings;if(bake.bake->contains("material"))bindings.push_back(bake.bake->at("material"));if(bake.bake->contains("materials"))for(const auto& b:bake.bake->at("materials"))bindings.push_back(b);
        for(const auto& b:bindings){protect(b.at("graph").get<std::string>());auto recipe=loadMaterialRecipe(b,b.contains("height")?std::vector<std::string>{b["height"].get<std::string>()}:std::vector<std::string>{});auto expandedRecipe=expandImports(recipe.document,recipe.base,recipe.document.value("seed",42),recipe.document.value("tile",false));for(const auto& file:expandedRecipe.files)protect(file);for(const auto& node:expandedRecipe.nodes)if(node["op"]=="image")protect(recipe.base/node["path"].get<std::string>());}
    }
...
Json Graph::render(const std::function<void(const Output&, const ImagePtr&)>& sink) {
    ...
    for(const auto& detail:stats["output_details"])if(detail.value("type",std::string{})=="export_bake")stats["peak_buffer_mb"]=std::max(stats["peak_buffer_mb"].get<double>(),detail.at("estimated_peak_working_mb").get<double>());
